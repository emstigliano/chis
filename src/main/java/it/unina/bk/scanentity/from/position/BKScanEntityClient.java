package it.unina.bk.scanentity.from.position;
...
public class BKScanEntityClient extends AbstractMediator {
	...
	public boolean mediate(MessageContext context) {
		...
		try {
			...
			if (context.getEnvelope().getBody().getFirstElement()
					.getFirstChildWithName(new QName("municipality")).getText() != null
					&& !(context.getEnvelope().getBody().getFirstElement()
							.getFirstChildWithName(new QName("municipality"))
							.getText().equals(""))) {
				var = "COMUNE";
				val = context.getEnvelope().getBody().getFirstElement()
						.getFirstChildWithName(new QName("municipality"))
						.getText();
			} else if (context.getEnvelope().getBody().getFirstElement()
					.getFirstChildWithName(new QName("province")).getText() != null
					&& !(context.getEnvelope().getBody().getFirstElement()
							.getFirstChildWithName(new QName("province"))
							.getText().equals(""))) {
				var = "PROVINCIA";
				val = context.getEnvelope().getBody().getFirstElement()
						.getFirstChildWithName(new QName("province")).getText();
			} else if (context.getEnvelope().getBody().getFirstElement()
					.getFirstChildWithName(new QName("region")).getText() != null
					&& !(context.getEnvelope().getBody().getFirstElement()
							.getFirstChildWithName(new QName("region"))
							.getText().equals(""))) {
				var = "REGIONE";
				val = context.getEnvelope().getBody().getFirstElement()
						.getFirstChildWithName(new QName("region")).getText();
			} else {
				var = "";
				val = "";
			}
			...
			arraySelectColumns.put("DC,DC:" + var.toUpperCase());

			JSONArray arrayFiltri = new JSONArray();
			JSONObject filtro = new JSONObject();
			filtro.put("fam", "DC");
			filtro.put("col", "DC:" + var.toUpperCase());
